...
import logo from "../assets/twineverse-logo.png";
        ...
        <div className="lg:col-span-3">
          ...
          <div className="flex flex-col md:flex-row items-center justify-between mb-6">
            <input
              ...
              className="mb-4 md:mb-0 md:mr-4 px-4 py-2 border rounded w-full md:w-1/2 dark:bg-gray-700 dark:border-gray-600 dark:text-white"
            />
            <select
              value={selectedCategory}
              onChange={(e) => setSelectedCategory(e.target.value)}
              className="px-4 py-2 border rounded w-full md:w-1/4 dark:bg-gray-700 dark:border-gray-600 dark:text-white"
            >
          ...
          <div className="grid md:grid-cols-2 gap-6">
            {filteredPosts.map((post) => (
              <article
                key={post.slug}
                className="bg-white dark:bg-gray-700 rounded-lg shadow-lg hover:shadow-xl transition-shadow duration-300 overflow-hidden"
              >
                {post.image && (
                  <img
                    src={post.image}
                ...
                <div className="p-6">
                  ...
                  <Link
                    to={`/blog/${post.slug}`}
                    className="text-blue-600 dark:text-blue-400 mt-2 inline-block hover:underline font-semibold"
                  >
                    Read More →
                  </Link>
                </div>
